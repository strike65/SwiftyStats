...
/// - Parameter c: direction
/// - Throws: SSSwiftyStatsError if c <= 0
public func pdfVonMisesDist(x: Double!, mean m: Double!, concentration c: Double!) throws -> Double {
    if c <= 0.0 {
        #if os(macOS) || os(iOS)
...
fileprivate func d_modp(x: Double, y: Double) -> Double {
    var value: Double
    if y.isZero {
        #if os(macOS) || os(iOS)
...
private func integrandVonMises(x: Double!, mean: Double!, concentration: Double!, pß: Double!, p1: Double!) -> Double {
    return try! pdfVonMisesDist(x: x, mean: mean, concentration: concentration)
}
...
/// - Throws: SSSwiftyStatsError if c <= 0
public func cdfVonMisesDist(x: Double!, mean m: Double!, concentration c: Double!, useExpIntegration: Bool = false) throws -> Double {
    if c <= 0.0 {
        #if os(macOS) || os(iOS)
    ...
    if ( xx < 0 ) {
        ...
        if useExpIntegration {
            ...
            _result = integrate(integrand: integrandVonMises, parameters: [aa, c, 0.0, 0.0], leftLimit: 0, rightLimit: xx, maxAbsError: 1E-14, numberOfEvaluations: &eval, estimatedError: &error)
            print("error: \(error)")
        }
        else {
            let a1 = 12.0
...
public func quantileVonMisesDist(p: Double!, mean m: Double!, concentration c: Double!) throws -> Double {
    if c <= 0.0 {
        #if os(macOS) || os(iOS)
    ...
    {
        do {
            _test = try cdfVonMisesDist(x: mVal, mean: m, concentration: c)
        }
        catch {
            throw error
        }
